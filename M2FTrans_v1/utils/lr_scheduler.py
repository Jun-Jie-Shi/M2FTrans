...
class LR_Scheduler(object):
    ...
    def __call__(self, optimizer, epoch):
        if self.mode == 'poly':
            ...
            if epoch < 100:
                now_lr = round(self.lr * (((np.float32(epoch))/(np.float32(100.0)))),8)
            else:
                now_lr = round(self.lr * np.power(1 - (np.float32(epoch) - np.float32(100.0))/(np.float32(self.num_epochs)-np.float32(100.0)), 0.9), 8)
